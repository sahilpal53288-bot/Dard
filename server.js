...
app.post("/api/wingo-play", (req,res)=>{
  ...
  const result = Math.floor(Math.random()*50)+1;
  let win = false;
  if(guess===result){ user.wallet += bet*2; win=true; } else user.wallet-=bet;
...
app.post("/api/slot-play", (req,res)=>{
  ...
  const spin = Math.floor(Math.random()*100);
  let win=false;
  if(spin>50){ user.wallet += bet*1.5; win=true; } else user.wallet-=bet;
...
app.post("/api/avatar-play", (req,res)=>{
  ...
  const roll = Math.floor(Math.random()*6)+1;
  let win = roll===6;
  if(win) user.wallet += bet*3; else user.wallet-=bet;
